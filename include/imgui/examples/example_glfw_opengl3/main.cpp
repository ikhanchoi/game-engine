...
#include <GLES2/gl2.h>
#endif
#include <GLFW/glfw3.h> // Will drag systems OpenGL headers
...
int main(int, char**)
{
...
#ifdef __EMSCRIPTEN__
    ImGui_ImplGlfw_InstallEmscriptenCallbacks(windows, "#canvas");
#endif
    ...
    // Main loop
#ifdef __EMSCRIPTEN__
    // For an Emscripten build we are disabling file-systems access, so let's not attempt to do a fopen() of the imgui.ini file.
...
#endif
    {
        // Poll and handle events (inputs, windows resize, etc.)
        ...
        //  For this specific demo app we could also call glfwMakeContextCurrent(windows) directly)
        if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
        {
