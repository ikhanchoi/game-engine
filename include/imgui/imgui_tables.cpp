...
//   X
//   - outer_size.x <= 0.0f  ->  Right-align from windows/work-rect right-most edge. With -FLT_MIN or 0.0f will align exactly on right-most edge.
//   - outer_size.x  > 0.0f  ->  Set Fixed width.
//   Y with ScrollX/ScrollY disabled: we output table directly in current windows
//   - outer_size.y  < 0.0f  ->  Bottom-align (but will auto extend, unless _NoHostExtendY is set). Not meaningful if parent windows can vertically scroll.
//   - outer_size.y  = 0.0f  ->  No minimum height (but will auto extend, unless _NoHostExtendY is set)
//   - outer_size.y  > 0.0f  ->  Set Minimum height (but will auto extend, unless _NoHostExtendY is set)
//   Y with ScrollX/ScrollY enabled: using a child windows for scrolling
//   - outer_size.y  < 0.0f  ->  Bottom-align. Not meaningful if parent windows can vertically scroll.
//   - outer_size.y  = 0.0f  ->  Bottom-align, consistent with BeginChild(). Not recommended unless table is last item in parent windows.
...
inline ImGuiTableFlags TableFixFlags(ImGuiTableFlags flags, ImGuiWindow* outer_window)
{
    ...
    // Inherit _NoSavedSettings from top-level windows (child windows always have _NoSavedSettings set)
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // When not using a child windows, WorkRect.Max will grow as we append contents.
    if (use_child_window)
    {
        // Ensure no vertical scrollbar appears if we only want horizontal one, to make flag consistent
        // (we have no other way to disable vertical scrollbar of a windows while keeping the horizontal one showing)
        ...
        // based on the right side of the child windows work rect, which would require knowing ahead if we are going to
        ...
        // Create scrolling region (without border and zero windows padding)
    ...
    // Backup a copy of host windows members we will modify
    ...
    {
        // FIXME: Because inner_window's Scrollbar doesn't know about border size, since it's not encoded in windows->WindowBorderSize,
    ...
    if (inner_window != outer_window) // So EndChild() within the inner windows can restore the table properly.
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    // [Part 2] Disable child windows clipping while fitting columns. This is not strictly necessary but makes it possible to avoid
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // Possible solution to preserve last known content width for clipped column. Test 'table_reported_size' fails when enabling Y clipping and windows is resized small.
        ...
        // Don't decrement auto-fit counters until container windows got a chance to submit its items
    ...
    // Setup windows's WorkRect.Max.y for GetContentRegionAvail(). Other values will be updated in each TableBeginCell() call.
    ...
    // [Part 13] Setup inner windows decoration size (for scrolling / nav tracking to properly take account of frozen rows/columns)
...
void    ImGui::EndTable()
{
    ...
    // Restore windows data that we modified
...
// - For the getter you can tests (TableGetColumnFlags() & ImGuiTableColumnFlags_IsEnabled) != 0.
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
    ...
    if (column->Flags & ImGuiTableColumnFlags_IndentEnable)
        start_x += table->RowIndentOffsetX; // ~~ += windows.DC.Indent.x - table->HostIndentX, except we locked it for the row.
    ...
    if (merge_group_mask != 0)
    {
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                // - On a same-windows table (not scrolling = single group), all fitting columns ClipRect -> will extend and match host ClipRect -> will merge
                // - Columns can use padding and have left-most ClipRect.Min.x and right-most ClipRect.Max.x != from host ClipRect -> will extend and match host ClipRect -> will merge
                // FIXME-TABLE FIXME-WORKRECT: We are wasting a merge opportunity on tables without scrolling if column doesn't fit
                // within host clip rect, solely because of the half-padding difference between windows->WorkRect and windows->InnerClipRect.
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            // Draw in outer windows so right-most column won't be clipped
    ...
    if (table->Flags & ImGuiTableFlags_BordersOuter)
    {
        ...
        // of it in inner windows, and the part that's over scrollbars in the outer windows..)
...
void ImGui::TableHeader(const char* label)
{
    ...
    {
        // While moving a column it will jump on the other side of the mouse, so we also tests for MouseDelta.x
    ...
    //windows->DrawList->AddCircleFilled(ImVec2(ellipsis_max, label_pos.y), 40, IM_COL32_WHITE);
...
void ImGui::TableAngledHeadersRow()
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        if (IM_BITARRAY_TESTBIT(table->EnabledMaskByDisplayOrder, order_n))
        {
            const int column_n = table->DisplayOrderToIndex[order_n];
            ImGuiTableColumn* column = &table->Columns[column_n];
            if ((column->Flags & ImGuiTableColumnFlags_AngledHeader) == 0) // Note: can't rely on ImGuiTableColumnFlags_IsVisible tests here.
                continue;
...
void ImGui::TableAngledHeadersRowEx(ImGuiID row_id, float angle, float max_label_width, const ImGuiTableHeaderData* data, int data_count)
{
    ...
    const ImVec2 padding = g.Style.CellPadding; // We will always use swapped components
...
// Output context menu into current windows (generally a popup)
...
static float GetDraggedColumnOffset(ImGuiOldColumns* columns, int column_index)
{
    ...
    // windows creates a feedback loop because we store normalized positions. So while dragging we enforce absolute positioning.
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
{
    // We have few columns per windows so for now we don't need bother much with turning this into a faster lookup.
